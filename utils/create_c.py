...
def plasma_fractal(mapsize=96, wibbledecay=3):
    """
...
def fog(x, severity=1):
    ...
    x = np.array(x) / 255.
    max_val = x.max()
    x += c[0] * plasma_fractal(wibbledecay=c[1])[:96, :96][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    frost = cv2.resize(frost, (0, 0), fx=1, fy=1)
    # randomly crop and convert to rgb
    x_start, y_start = np.random.randint(0, frost.shape[0] - 96), np.random.randint(0, frost.shape[1] - 96)
    frost = frost[x_start:x_start + 96, y_start:y_start + 96][..., [2, 1, 0]]
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(96, 96, 1) * 1.5 + 0.5)
...
def pixelate(x, severity=1):
    ...
    x = x.resize((int(96 * c), int(96 * c)), PILImage.BOX)
    x = x.resize((96, 96), PILImage.BOX)
...
print('hello')
test_data = dset.STL10('/media/lord-of-the-gay/68334fe0-2b83-45d6-98e3-76904bf08127/home/namjuhyeon/Desktop/LAB/common material/Dataset Collection/stl10',
                       split='test', download=True, transform=transforms.Compose(test_transform_list))
test_loader = u_data.DataLoader(test_data, batch_size=1, shuffle=False)
...
for method_name in d.keys():
    ...
    for severity in range(1, 6):
        ...
        for image, target in tqdm.tqdm(test_loader) :
            image = np.asarray(np.transpose(image[0].cpu().detach().numpy(), (1, 2, 0)) * 255, dtype=np.uint8)
            labels.append(target.item())
            stl_c.append(np.uint8(corruption(convert_img(image))))
    np.save('/media/lord-of-the-gay/68334fe0-2b83-45d6-98e3-76904bf08127/home/namjuhyeon/Downloads/STL-10-C/' + d[method_name].__name__ + '.npy',
            np.array(stl_c).astype(np.uint8))

    np.save('/media/lord-of-the-gay/68334fe0-2b83-45d6-98e3-76904bf08127/home/namjuhyeon/Downloads/target.npy',
    ...
    #
    #     np.save('/media/lord-of-the-gay/68334fe0-2b83-45d6-98e3-76904bf08127/home/namjuhyeon/Downloads/labels.npy',
    #             np.array(labels).astype(np.uint8))